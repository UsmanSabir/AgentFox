<script lang="ts">
  ...
  async function load() {
    ...
    error = null;
    try {
      [status, proposals, executions, reconciliation, events] = await Promise.all([
        api.trading.status(), api.trading.proposals(), api.trading.executions(),
        api.trading.reconciliation(), api.trading.events()
      ]);
    } catch (e) {
